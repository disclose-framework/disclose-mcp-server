...
async def check_signal_coverage(domain: str) -> str:
            ...
            try:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        return json.dumps({"domain": domain, "error": f"Found a file at {url} but it could not be parsed as valid JSON."}, indent=2)
            except httpx.RequestError as e:
                return json.dumps({"domain": domain, "error": f"Network error reaching {url}: {e}"}, indent=2)
        if data is None:
            try:
                response = await client.get(base)
                if response.status_code == 200:
    ...
    populated_pct = round((len(present) - len(present_with_null_value)) / len(V1_SIGNALS) * 100, 1)
    report = {
        "domain": domain,
